    ...
    public void start(){
        ...
        for (int i = 1; i <= this.generatedValues.workers; i++ ) {
            if (workersInSection < quotient) {
                Producers producer = new Producers(i,this.generatedValues.getWorkCapacity(works[section]),wages[section],0,works[section],true, wareHouses[section], "TRABAJANDO", true,24,2, this.counter, this.interfaz);
                producer.start();
                workersInSection += 1;
            } else {
                section += 1;
        ...
        for (int i=1; i < quotient + remainder; i++) {
            Assemblers assembler = new Assemblers(this.generatedValues.getGeneratorNumber() - quotient + i,1,50,0,true, true, "TRABAJANDO", wareHouses[0], 
                                                    wareHouses[2], wareHouses[3], wareHouses[1], wareHouses[4], 2, 24, this.counter, this.interfaz);
            assembler.start();
        }
        
        
        Manager manager = new Manager(1, 40, "", 24.0, 0, this.counter, 2,2,2,2, this.interfaz);
        Supervisor supervisor = new Supervisor(5, manager,40, 24, "", this.counter, (int) Math.round(this.standardCarPrice), this.interfaz);
